        ...
        if Cluster_allocation[i] == 'NOT':
            ...
            if len(cl_constraint_marker[i]) != 0:
                for cl in cl_constraint_marker[i]:
                    ...
                    while len(cl_copy) > 0:
                        if len(cl_copy) == 1:
                            Cluster_allocation[cl_copy[0]] = cl_nearest_center[cl_copy[0]]
                            break
                        ...
                        for m in range(len(cl_copy)):
                            # sum_cl_copy_match += euclidean_distanceeuclidean_distanc(embedding_dict[str(cl_copy[m])],centers_mapping[match_r[m]])
                        ...
                        for c in cl_copy:
                            ...
                            gy = sum_cl_copy_match - sum_cl_copy_remove_c_match - distance_cc[(c,cl_nearest_center[c])]
                            gy_list.append(gy)
                        max_gy = max(gy_list)
                        if max_gy < num_y_list[gy_list.index(max_gy)] * weight:
                            ...
                            break
                        Cluster_allocation[cl_copy[gy_list.index(max_gy)]] = cl_nearest_center[cl_copy[gy_list.index(max_gy)]]
        ...
        if i not in ml_labled_points:
            ...
            if len(ml_constraint_marker[i]) != 0:
                ...
                while len(mls_copy) > 0:
                    ...
                    g_list = []
                    for xj in mls_copy:
                        g = gj(mls_copy, xj, ml_nearest_center[xj], embedding_dict, sum_d, centers_mapping,
                               distance_mc)
                        g_list.append(g)
                    # print(f'g:{g_list}')
                    max_gj = max(g_list)
                    ...
                    if check_point:
                        ...
                        Cluster_allocation[mls_copy[g_list.index(max_gj)]] = ml_nearest_center[mls_copy[g_list.index(max_gj)]]
...
def local_search(cls_,mls_,data,embedding_dict,y_true,weight):
    ...
    for sd in sd_list:
        ...
        for iteration in range(max_iters):
            ...
            # If cluster centers don't change, algorithm has converged
    ...
    print('Average case:')
    print(f'clustering:ACC_mean: {ACC_mean:.2f} ;ACC_std:{ACC_std:.2f} ;NMI:{NMI_mean:.2f};NMI_std:{NMI_std:.2f};ARI:{ARI_mean:.2f};ARI_std:{ARI_std:.2f}')
...
print(f'==Check normalization:{check_is_normalized}')
...
for pro,constrains in result.items():
    ...
    mls = []
    for ml in mls_dict.values():
        temp = []
        for m in ml:
            if int(m) < len(data):
                temp.append(int(m))
        mls.append(temp)
    mls = [ml for ml in mls if len(ml) >= 2]

    print(f"======Constraint proportion: {pro}=====")
    print(f"==cls:{cls}")
    print(f"==mls:{mls}")
    result_dict = local_search(cls,mls,data, embedding_dict,y_true,P)
